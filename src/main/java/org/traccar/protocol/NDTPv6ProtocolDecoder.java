...
public class NDTPv6ProtocolDecoder extends BaseProtocolDecoder {
  ...
  public NDTPv6ProtocolDecoder(Protocol protocol) {
    super(protocol);
    adc1Ratio = Context.getConfig().getDouble("ndtpv6.adc1Ratio", 1);
    adc2Ratio = Context.getConfig().getDouble("ndtpv6.adc2Ratio", 1);
    adc3Ratio = Context.getConfig().getDouble("ndtpv6.adc3Ratio", 1);
    adc4Ratio = Context.getConfig().getDouble("ndtpv6.adc4Ratio", 1);
  }

  private static final int NPH_PACKET_HEADER_SIZE = 15;
  ...
  private static final int NPH_SRV_FILE_TRANSFER = 3;
  private static final int NPH_SRV_CLIENT_LIST = 4;
  private static final int NPH_SRV_EXTERNAL_DEVICE = 5;
  private static final int NPH_SRV_DEBUG = 6;
  ...
  private static final int NPH_SGC_CONN_AUTH_STRING = 101;
  private static final int NPH_SGC_SERVICE_REQUEST = 110;
  private static final int NPH_SGC_SERVICES_REQUEST = 111;
  private static final int NPH_SGC_SERVICES = 112;
  private static final int NPH_SGC_PEER_DESC_REQUEST = 120;
  private static final int NPH_SGC_PEER_DESC = 121;

  /* NPH_SRV_NAVDATA packets */
  private static final int NPH_SND_RESULT = NPH_RESULT;
  private static final int NPH_SND_HISTORY = 100;
  private static final int NPH_SND_REALTIME = 101;
  ...
  private void decodeData(ByteBuf buf, Position position, int dataType) {
    if (dataType == NPH_SRV_NAVDATA) {
      ...
      if (cellType == ADDITIONAL_NAV_DATA && cellNumber == 0) {
        ...
        int output = buf.readUnsignedByte(); // Значение цифровых входов
